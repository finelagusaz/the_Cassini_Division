...
- シェル `master`: サトウ Ｍ 作のフリーシェル「しろ」（配布サイト「TCG」、現在は閲覧不可）。改変した画像の配布や商用利用の可否は、手元の資料からは確かめられなかった（作者に確認）。確かめるまで画像は編集しない。
- シェル `alternative_cassis`: 吉野桜 作のフリーシェル「Ｐ＆Ｗ＆Ｆ ver.1.3」を、使用の快諾を得て改造したもの。再改変や再配布の条件は未確認（作者に確認）。
...
| `\1` | 間隙（追加シェル「Gemini」ではカストール） | 通常のシェルでは台詞が無く、トークの頭で `\1\s[10]` を指定して佇むだけ。Gemini のときだけ、カストールとして話す（`\0` 側はポルックスと呼ばれる。下の「Gemini の掛け合い」）。口調はカシスと同じ系統（「〜わ」「〜かしら」「〜わね」）。人物像の細部は作者に確認 | 通常は 10 空隙のみ。Gemini では 10 素、11 照れ、12 驚き、13 不安/胸を隠す、14 落ち込み、15 笑い、16 目とじ、17 怒り、18 冷笑、19 恥じらい、40・41 スカート（`shell/gemini/surfacetable.txt` の 2010 番台に当たる） |

- 当たり判定（`surfaces.txt` の `collision`）: `Head`、`Lip`、`Bust`、`Skirt`（すわり時は `Skirt_s`）、`Wing`
- トークで使わないサーフェス:
  - 1000〜1009、1030〜1032（すわり）: 手で書かない。`OnTranslate` の `sfcnv` が、すわりモードのとき全ての `\s[N]` に 1000 を自動で足す。
